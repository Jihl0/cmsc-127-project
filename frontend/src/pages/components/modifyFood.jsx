...
function ModifyFood() {
    ...
    const handleUpdateFoodItem = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:5000/update-food-item/${selectedFoodItem.FoodItemID}`, {
                name,
                price,
                averageRating,
                foodType
    ...
    const handleDeleteFoodItem = async (e) => {
        ...
        try {
            await axios.delete(`http://localhost:5000/delete-food-item/${selectedFoodItem.FoodItemID}`);
    ...
    return (
        <div className='page-container'>
            <h1>Modify Food Items</h1>

            <form onSubmit={handleAddFoodItem}>
                <h2>Add Food Item</h2>
                <select onChange={(e) => setEstablishmentId(e.target.value)} value={establishmentId}>
            ...
            <form onSubmit={handleUpdateFoodItem}>
                ...
                <select onChange={handleFoodItemSelect} value={selectedFoodItem?.FoodItemID || ''}>
                    <option value='' disabled>Select a food item to update</option>
                    {foodItems.map((item) => (
            ...
            <form onSubmit={handleDeleteFoodItem}>
                ...
                <select onChange={handleFoodItemSelect} value={selectedFoodItem?.FoodItemID || ''}>
                    <option value='' disabled>Select a food item to delete</option>
                    {foodItems.map((item) => (
                        <option key={item.FoodItemID} value={item.FoodItemID}>
                            {item.FoodItemName}
                        </option>
