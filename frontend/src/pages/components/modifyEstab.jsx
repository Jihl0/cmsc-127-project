...
export default function ModifyEstab() {
  ...
  const handleUpdateEstablishment = async (e) => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/update-establishment/${selectedEstablishment.EstablishmentID}`,
        {
          name,
          averageRating,
          address,
  ...
  const handleDeleteEstablishment = async (e) => {
    ...
    try {
      await axios.delete(
        `http://localhost:5000/delete-establishment/${selectedEstablishment.EstablishmentID}`
      );
  ...
  const handleEstablishmentSelect = (e) => {
    ...
    const establishment = establishments.find(
      (est) => est.EstablishmentID == establishmentId
    );
    ...
    <div className="modifyestab-container">
      ...
      <form onSubmit={handleUpdateEstablishment}>
        ...
        <div className="same-line">
          <select
            ...
            value={selectedEstablishment?.EstablishmentID || ''}
          >
            <option value="" disabled>
              Select an establishment to update
            </option>
      ...
      <form onSubmit={handleDeleteEstablishment}>
        ...
        <div className="same-line">
          ...
          >
            <option value="" disabled>
              Select an establishment to delete
            </option>
            {establishments.map((establishment) => (
              <option
                key={establishment.EstablishmentID}
          ...
          {selectedEstablishment && <button type="submit">Delete</button>}
        </div>
      </form>
    </div>
  );
